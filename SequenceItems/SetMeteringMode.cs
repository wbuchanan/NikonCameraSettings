...
namespace NikonCameraSettings.SequenceItems {
    ...
    public class SetMeteringMode : SequenceItem, IValidatable {
        ...
        private void SetMeteringModeSettingsList() {
            ...
            var e = theCam.GetEnum(eNkMAIDCapability.kNkMAIDCapability_MeteringMode);
            var list = new List<string>();
            for (int i = 0; i < e.Length; i++) list.Add(e[i].ToString());
            MeteringModeSettings = list;
        }
        ...
        public override Task Execute(IProgress<ApplicationStatus> progress, CancellationToken token) {
            var e = theCam.GetEnum(eNkMAIDCapability.kNkMAIDCapability_MeteringMode);
            e.Index = meteringModeSettings.IndexOf(selectedMeteringModeSetting);
            theCam.SetEnum(eNkMAIDCapability.kNkMAIDCapability_MeteringMode, e);
            return Task.CompletedTask;
        }
    }
}
